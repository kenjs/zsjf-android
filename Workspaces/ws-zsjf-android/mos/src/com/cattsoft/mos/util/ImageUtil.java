...
public class ImageUtil {
	...
	public static byte[] compressImage(Bitmap image,Context context) {  
	...
	public static Bitmap compressBitMap(Bitmap image,Context context) {  
        ByteArrayOutputStream baos1 = new ByteArrayOutputStream();  
        image.compress(Bitmap.CompressFormat.JPEG, 100, baos1);//质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中  
        int options = 100;   
        while ( baos1.toByteArray().length / 1024>100) {  //循环判断如果压缩后图片是否大于100kb,大于继续压缩         
        ...
        //Toast.makeText(context, "压缩后大小:"+ baos1.toByteArray().length/1024, Toast.LENGTH_SHORT).show();
        ByteArrayInputStream isBm = new ByteArrayInputStream(baos1.toByteArray());//把压缩后的数据baos存放到ByteArrayInputStream中  
        Bitmap bitmap = BitmapFactory.decodeStream(isBm, null, null);//把ByteArrayInputStream数据生成图片  
        return bitmap;  
    }  
	public static Bitmap compressBitMapBySize(Bitmap image,Context context) {  
		    ...
		    return compressBitMap(bitmap,context);//压缩好比例大小后再进行质量压缩  
		}  
